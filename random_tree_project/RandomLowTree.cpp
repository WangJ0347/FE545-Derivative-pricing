    ...
    for (unsigned long i = 0; i < NumberOfPaths; ++i) {
        ...
        for (unsigned long j = 1; j <= Steps ; ++j) {
            ...
            for (unsigned long k = 0; k < pow(b,index); k++ ){
                double h = TheProduct.FinalPayOff(TheTree[index][k].first);
                double V_u = TheTree[index+1][3*k].second;
                double V_m = TheTree[index+1][3*k+1].second;
                double V_l = TheTree[index+1][3*k+2].second;
                double U = h >= 0.5*(V_m+V_l) ? h:V_u;
                double M = h >= 0.5*(V_u+V_l) ? h:V_m;
                double L = h >= 0.5*(V_u+V_m) ? h:V_l;

                TheTree[index][k].second = (U + M + L)/b;
            }
        }
